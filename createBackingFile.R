...
p <- arg_parser('Create bigSNPR rds/bk (backing) files from PLINK (.bed) or BGEN (.bgen)) files')
p <- add_argument(p, '--file-input', help='The input file (.bed/.bgen file ending)')
p <- add_argument(p, '--file-output', help='The output basename of files')
p <- add_argument(p, '--file-snp-list', help='Text file containing list of SNPs to extract from .bgen file (format <chr>_<pos>_<a1>_<a2>)', default = NULL)
