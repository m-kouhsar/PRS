...
par <- arg_parser('Calculate polygenic risk scores using ldpred2')
...
par <- add_argument(par, "--ld-file", help="LD reference files, split per chromosome; chr label should be indicated by '@' symbol")
par <- add_argument(par, "--ld-meta-file", help="list of variants in --ld-file")
par <- add_argument(par, "--trait-type", help="Type of trait: b for binary q for quantitative")
...
par <- add_argument(par, "--col-A2", help="Noneffective allele column", default="a0", nargs=1)
par <- add_argument(par, "--col-bp", help="SNP position column. Will be ignored if --merge-by-rsid flag is used", default="pos", nargs=1)
...
par <- add_argument(par, "--col-n-cases", help="Nr cases when phenotype is binary. For calculating effective sample size. Override with --n-cases", nargs=1)
par <- add_argument(par, "--col-n-controls", help="Nr controls when phenotype is binary. For calculating effective sample size. Override with --n-controls", nargs=1)
...
par <- add_argument(par, "--n-cases", help="Nr cases when phenotype is binary. For calculating effective sample size. if unavailable in sumstats (--col-n-cases)", nargs=1)
par <- add_argument(par, "--n-controls", help="Nr controls when phenotype is binary. For calculating effective sample size. if unavailable in sumstats (--col-n-controls)", nargs=1)
# Polygenic score name
...
par <- add_argument(par, '--set-seed', help="Set a seed for reproducibility", default = 123,nargs=1)
...
fileOutput <- paste0(parsed$out,".PRS.",argLdpredMode)
...
colN <- parsed$col_n
colNCases <- parsed$col_n_cases
colNControls <- parsed$col_n_controls
traitType <- parsed$trait_type
...
if (mergeByRsid) {
  ...
  colSumstatToGeno <- c("chr",  "snp",  "a1",  "a0",  "beta",  "beta_se")
} else {
  colSumstatsOld <- c(colChr, colSNPID, colBP, colA1, colA2, colStat, colStatSE)
  colSumstatToGeno <- c("chr",  "snp",  "pos",  "a1",  "a0",  "beta",  "beta_se")
}
...
if (!is.na(colNCases)) colNCases <- tolower(colNCases)
if (!is.na(colNControls)) colNControls <- tolower(colNControls)
sumstats$n_eff <- getEffectiveSampleSize(sumstats, traitType,effectiveSampleSize=argEffectiveSampleSize, cases=argNCases, 
                                         controls=argNControls, colN=colN, colNCases=colNCases , colNControls = colNControls)
...
write.csv(obj.bigSNP$fam , file = paste0(fileOutput,".csv"),row.names = F)
